...
def login_user(request):
    ...
    if request.method == 'POST':
        ...
        logger.debug(f"Trying to authenticate user {user.email} with password {password}")
        ...
        if user.role == User.ROLE_GARAGE:
            messages.error(request, 'Vous êtes un garage et ne pouvez pas vous connecter ici.')
            return redirect(reverse('login') + f'?type={type_client}')
...
def send_mail_after_registration(email, token):
    ...
    email_template_name = render_to_string("emailConfirmation.html", {'message': message, 'content': context})
    email_from = settings.EMAIL_HOST_USER
    recipient_list = [email]
    
    try:
        send_mail(subject, message, email_from, recipient_list, html_message=email_template_name)
